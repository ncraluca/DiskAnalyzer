...
void Help(char *buf) {
  ...
  sprintf(buf, "%s",
          ...
          "\t-p, --priority\t\tset priority for the new analysis (works only "
          "with -a argument)\n"
          "\t-S, --suspend <id>\tsuspend task with id <id>\n"
          "\t-R, --resume <id>\tresume task with <id>\n"
          "\t-r, --remove <id>\tremove the analysis with the given <id>\n"
          "\t-i, --info <id>\t\tprint status about the analysis with <id> "
          "(pending, progress, done)\n"
          "\t-l, --list\t\tlist all analysis tasks, with their ID and the "
          "corresponding root path\n"
          "\t-p, --print <id>\tprint analysis report for those tasks that are "
          "'done'\n");
}
...
void Add(char *path, char *priority, char *buf) {
  ...
  if (existing_task == -1) {
    ...
    details->id = task_id; // id-ul task-ului de analizat, id-ul unui thread =
                           // id-ul task-ului
...
void Suspend(int id, char *buf) {
  ...
  // ideea e ca trebuie sa cautam in lista de threaduri threadul cu id-ul id si
  // sa-l suspendam
  // eventual ii punem un status temporar de suspended
  // IDEE: cream un nou camp in structura thr_node care sa fie un bool, un int:
  // int suspended = 0(0 inseamna ca nu e suspendat, 1 inseamna ca e suspendat)
  // cand facem suspend, punem suspended = 1, iar cand facem resume punem
  // suspended = 0 cand facem suspend, verificam daca suspended == 0, daca da,
  // atunci facem suspend, daca nu, nu facem nimic cand facem resume, verificam
  // daca suspended == 1, daca da, atunci facem resume, daca nu, nu facem nimic
  // in disk analyzer sau in programul care executa thread-urile ar trb sa se
  // verifice mereu daca el e suspendat sau nu (are un while(1) in care se
  // verifica daca e suspendat sau nu, daca nu e suspendat, face ce are de
  // facut)

  /*//tot nu e f buna ca sterge de tot thread-ul
  struct thr_node *thread_node = list_find_by_key(list_head, id);
  if (thread_node == NULL) {
      sprintf(buf, "No existing analysis for task ID %d, there is nothing to
  suspend", id); debug_daemon(buf); } else { thread_node->suspended = 1;
...
void Resume(int id, char *buf) {
  ...
  /*ideea e ca trebuie sa cautam in lista de threaduri threadul cu id-ul id si
  sa-l resume(sa il scoata din suspend) struct thr_node *thread_node =
  list_find_by_key(list_head, id); if (thread_node == NULL) { sprintf(buf, "No
  existing analysis for task ID %d, there is nothing to suspend", id);
      debug_daemon(buf);
  } else {
...
// functie care primeste instructiuni de la utilizator si apeleaza functiile
// corespunzatoare
