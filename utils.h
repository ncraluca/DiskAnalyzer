...
void create_directory(const char *given_path) {
  ...
  if (stat(given_path, &st) == -1) { // if the file from the path doesn't exist
    mkdir("/tmp/disk-analyzer",
          0777); // create the directory with 0777 permission =
...
void dir_hash_init(struct directory *m, int size) {
  m->size = size;
  m->content =
      (struct file_directory **)malloc(size * sizeof(struct file_directory *));
...
void dir_hash_delete(struct directory *m, int id) {
  struct file_directory *nod =
      (struct file_direcory *)malloc(sizeof(struct file_directory));
  nod = m->content[id % m->size];
  struct file_directory *prev =
      (struct file_directory *)malloc(sizeof(struct file_directory));
